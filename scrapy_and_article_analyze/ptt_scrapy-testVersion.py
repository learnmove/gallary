...
import re
import requests
from basic_tools import exception
...
class board(thr):
    ...
    def run_content(self):
        while 1:#因為有設中斷條件了，所以直接無限迴圈
            ...
            soup = bs(req.get(self.url, headers = headers).text,'html.parser')
...
#這裡開始為主執行緒，尋找ptt上各大討論版塊網址並發配給board執行緒
...
soup = bs(req.post('https://www.ptt.cc/ask/over18', headers = headers,data = payload).text,'html.parser')
